...
- [x] 8.1 로그인/회원가입 화면 구현 (Vanilla JS + Tailwind, validation 클라이언트측 이메일형식/8자이상)
- [x] 8.2 JWT localStorage 저장/읽기/삭제 유틸리티 작성
- [x] 8.3 401 응답 시 자동 로그인 페이지 redirect 인터셉터 작성
- [x] 8.4 팀 선택 화면 구현 (팀 만들기, 초대코드 입력+합류, team_id NULL 강제 진입 로직)

## 9. 프론트엔드 - 칸반 화면

- [x] 9.1 칸반 3컬럼 레이아웃 구현 (TODO/DOING/DONE, empty state 포함)
- [x] 9.2 태스크 카드 생성 인라인 입력 구현 (담당자 선택 포함)
- [x] 9.3 HTML5 native drag & drop 구현 (드롭 시 PATCH /tasks/{id}/status 호출)
- [x] 9.4 카드 상세/수정 모달 구현 (제목/상태/담당자 수정, 삭제 버튼 - 권한 없으면 숨김)
- [x] 9.5 필터(전체/@me/미할당) UI 및 API 연동 구현
- [x] 9.6 768px 미만 모바일 반응형 구현 (컬럼 스와이프, 카드 길게 누르기 메뉴, FAB 버튼)

## 10. 프론트엔드 - 채팅 화면

- [x] 10.1 채팅 화면 레이아웃 구현 (말풍선, 발신자+시각 표시, empty state)
- [x] 10.2 5초 setInterval 폴링 구현 (`since=` 파라미터로 증분 조회)
- [x] 10.3 메시지 입력 및 1000자 카운터/전송 버튼 disable 구현
- [x] 10.4 본인 메시지 호버 삭제 메뉴 구현
- [x] 10.5 모바일 반응형 채팅 화면 구현 (풀스크린, 키보드 대응)
